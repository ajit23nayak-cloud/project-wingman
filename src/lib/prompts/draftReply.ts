// Email reply drafter. Verbatim port of convex/prompts/draftReply.ts minus
// the Segment import (v0 stub uses single-pool voice corpus, no segmentation
// — see voice-init route). When v1 brings segmentation back, the segment-
// matched voice-prime branch returns; for now, all callers pass no segment.
...
function buildSystemPrompt(
  ...
  const voicePrime =
    voiceSnippets.length === 0
      ...
      : `Match the voice in these recent reply samples:\n${voiceSnippets.join("\n---\n")}`;
